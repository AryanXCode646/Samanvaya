...
| **2. Sun-Angle Invariance** (Opposite illumination, $180^\circ$ shadow reversal) | **YES** | **YES** | **NO** | **NO** | `SUPPORTED (Synthetic)` / `DATA_REQUIRED` | Phase Congruency maximum moment feature extraction invariant to solar DC offset; validated on synthetic solar variation ($12^\circ \leftrightarrow 65^\circ$); flight verification pending real pairs. |
| **3. Scale Invariance** (Multi-octave scale bridging, up to $320\times$) | **YES** | **YES** | **NO** | **NO** | `SUPPORTED (Synthetic)` / `DATA_REQUIRED` | Fourier-Mellin log-polar coarse localization and multi-resolution pyramid bridge scale ratios; verified synthetically across octave shifts; pending multi-resolution real flight pairs. |
...
| **5. Uniform Distribution** (Non-clumping on source image) | **YES** | **YES** | **NO** | **NO** | `PROVEN (Code & Synthetic)` / `DATA_REQUIRED (Flight)` | Spatial selection operates strictly in `FULL_SOURCE_IMAGE` coordinates; 8x8 grid capping rebalances clustered features; verified by regression test. |
...
* **Scale Robustness**: `SUPPORTED` (Implemented and verified in synthetic simulations).
* **Illumination Robustness**: `SUPPORTED` (Phase Congruency zero-DC moment analysis verified).
* **Uniform Distribution**: `PROVEN (Code & Unit Tests)` / `READY_FOR_REAL`.
